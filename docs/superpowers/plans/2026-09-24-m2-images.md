...
- Every task's last step before committing is `dotnet format Transom.sln --verify-no-changes`
  (fixing anything it reports with `dotnet format Transom.sln` first) — this is part of every
  task's definition of done, not an extra step to remember only when dispatching. `build-windows`
  CI fails the whole PR on a format violation regardless of which task introduced it.
- When a task changes a member on a shared interface, it updates every implementation —
  including test doubles — in the same task/commit, never in a later one. CI builds the whole
  solution on every push, so a half-updated interface breaks every task's build in between.
...
If it reports changes, run `dotnet format Transom.sln` and re-verify before continuing.
...
**This task also updates `FakeBlogProvider`, `IBlogProvider`'s other implementation, in the same
commit** — per CLAUDE.md ("when an interface gains a member, update every implementation,
including test doubles, in the same commit"). `Transom.Core.Tests` doesn't build
`Transom.App.Tests`, so a Core-only test run can't catch a half-updated interface; `build-windows`
CI builds the whole solution and would fail on every subsequent task's push until this is fixed,
which is exactly the gap an earlier draft of this plan had (`FakeBlogProvider` was originally only
going to be updated in Task 8). Task 8 does NOT touch `FakeBlogProvider.cs` — it's done here.
...
- [ ] **Step 4: Update `FakeBlogProvider` in the same commit**

`IBlogProvider` just gained a member; its other implementation must gain it too, now, not in a
later task. Modify `tests/Transom.App.Tests/TestDoubles/FakeBlogProvider.cs`:
...
`OnUploadMedia` isn't consumed by any test yet — Task 8 is what writes tests using it — but the
member must exist now so the type keeps satisfying `IBlogProvider`. An unused settable property on
a test double is not a YAGNI violation: it's required for the type to compile at all.
...
Expected: the filtered Core tests PASS (all `MicroBlogProviderTests`, including the 3 new ones),
and the full-solution build succeeds — confirming `Transom.App.Tests` (which is Windows-only and
not exercised by `dotnet test tests/Transom.Core.Tests`) still compiles against the updated
`FakeBlogProvider`.
...
git add src/Transom.Core/Providers tests/Transom.Core.Tests/Providers/MicroBlogProviderTests.cs \
        tests/Transom.App.Tests/TestDoubles/FakeBlogProvider.cs
...
- Consumes: `IBlogProvider.UploadMediaAsync` (Task 3), `ComposerImageViewModel` (Task 7),
  `ComposerErrorMessages` (Task 5), `FakeBlogProvider.OnUploadMedia` (Task 3 — `FakeBlogProvider`
  already has `UploadMediaAsync`/`OnUploadMedia`; this task does not modify that file, only uses it).
...
```bash
git add src/Transom.App/ViewModels/ComposerViewModel.cs tests/Transom.App.Tests/ViewModels/ComposerViewModelTests.cs
...
1. Click **Add image**, pick a plain PNG. Confirm a thumbnail appears, the alt-text dialog pops up,
   ...
   Add a second image and click **Skip** instead — confirm the warning glyph shows, and that both
   the glyph itself and the tile's **Alt** button reopen the same dialog, pre-filled with whatever
   text is currently set (empty the first time), so alt text is editable at any point, not just at
   add time. Clear a saved alt text and save again — confirm the warning glyph returns.
